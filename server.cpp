...
char * pipe_display = "/tmp/myfifo";//FILE PATH OF PIPE
    ...
    char buffer[50] = "SERVER CHAT";
    int fifo1, fifo2, fifoDisplay, choice;

    fifo1 = open(pipe_one, O_RDONLY);
    if (fifo1 < 0)
    {
        cout << "Error opening pipe_one file" << endl;
        return 1;
    }
    fifo2 = open(pipe_two, O_WRONLY);
    if (fifo2 < 0)
    {
    ...
    else
    {
        do
        {
            ...
            if (choice == 1)
            {
                ...
                {
                    cout << endl << "Enter Text To display from Server: ";
